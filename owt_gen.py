...
class Predictor:
    ...
    def predict(self, input: str) -> str:
        # token_ids = self.tokenizer.encode(input) # My own tokenizer
        token_ids = self.tokenizer.encode(input).ids # HF Tokenizer returns Encoding object, need .ids to get list[int]
        ...
        print(f"[DEBUG] Next token ID: {next_token_id}")  # Debug
        decoded = self.tokenizer.decode([next_token_id])
        print(f"[DEBUG] Decoded (raw): {repr(decoded)}")  # Debug
        # ByteLevel decoder automatically handles space conversion, no need to replace
        return decoded

    def softmax_temp_scal(self, x: torch.Tensor, temperature: float = 0.7):
        # Higher temperature = more randomness, helps avoid repetition
        # Default changed from 0.5 to 1.0
    ...
    def top_p(self, probs: torch.Tensor, p: float = 0.95) -> int:
    ...
    def predict_all(self, input: str, max_gen: int):
        ...
        print("reply: ")
        while next not in ["<|endoftext|>"] and cnt < max_gen:
...
def main():
    ...
    with open("checkpoints/step_80000", "rb") as f:
    ...
    tokenizer = Tokenizer.from_file("outputs/owt_tokenizer/tokenizer_fixed.json")
    
    predictor = Predictor(model, tokenizer)
    
    predictor.predict_all("say two words and stop output", 500)
